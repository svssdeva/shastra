...
// Write an unsealed list of triangles as a binary STL. Normals are left as
// zero — three.js / Yantra recompute them on parse, and our voxelizer does
// not consult them at all (it ray-crosses against the triangle soup).
export function writeStl(path: string, tris: V[][]): void {
  ...
  console.log(`wrote ${path}, ${tris.length} tris, ${buf.byteLength} bytes`);
}

// Axis-aligned box, six faces, two tris each. Shared with gen-fins.ts and
// gen-lbracket.ts; outward normals are not enforced because we don't use them.
export function box(min: V, max: V): V[][] {
